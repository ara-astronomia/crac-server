...
class Telescope(ABC):
    def __init__(self, hostname: str = None, port: int = None) -> None:  # type: ignore
        ...
        self._polling = False
        self._jobs = deque()
        self._has_tracking_off_capability = config.Config.getBoolean(
            "tracking_off", "telescope"
        )
        self._connection_retry = 0
        self._flat_coordinate = AltazimutalCoords(
    ...
    def queue_set_speed(self, speed: TelescopeSpeed):
        if (
            speed is TelescopeSpeed.SPEED_NOT_TRACKING
            and not self.has_tracking_off_capability
        ):
    ...
    def queue_park(self):
        speed = (
            ...
            else TelescopeSpeed.SPEED_TRACKING
        )
    ...
    def is_below_curtains_area(self, alt: float) -> bool:
        max_secure_alt = config.Config.getFloat("max_secure_alt", "telescope")
        if max_secure_alt:
            return alt <= max_secure_alt
        else:
            return False
    ...
    def is_within_curtains_area(self) -> bool:
        return self.status in (TelescopeStatus.EAST, TelescopeStatus.WEST)
    ...
    def __read(self):
        ...
        while self._polling:
            ...
            try:
                ...
                self.eq_coords, self.aa_coords, self.speed, self.status = (
                    self.retrieve()
                )
            except Exception:
                ...
                self.__disconnect()
                sleeping = config.Config.getFloat("polling_interval", "telescope")
                if sleeping:
                    sleep(sleeping)
        else:
            self._reset()
    ...
    def _retrieve_aa_coords(self, eq_coords):
        if eq_coords:
            aa_coords = (
                self._radec2altaz(eq_coords, obstime=datetime.utcnow())
                if eq_coords
                else None
            )
            return aa_coords
    ...
    def _retrieve_status(self, aa_coords: AltazimutalCoords) -> TelescopeStatus:
        if not self._polling:
        ...
        elif self.__within_park_alt_range(
            aa_coords.alt
        ) and self.__within_park_az_range(aa_coords.az):
            return TelescopeStatus.PARKED
        elif self.__within_flat_alt_range(
            aa_coords.alt
        ) and self.__within_flat_az_range(aa_coords.az):
            ...
            if config.Config.getInt("azNE", "azimut") > aa_coords.az:
                ...
                config.Config.getInt("azSW", "azimut")
                < aa_coords.az
                <= config.Config.getInt("azNW", "azimut")
            ):
                return TelescopeStatus.WEST
            elif (
                config.Config.getInt("azNE", "azimut")
                <= aa_coords.az
                <= config.Config.getInt("azSE", "azimut")
            ):
    ...
    def _calculate_eq_coords_of_park_position(
    ...
    def _calculate_telescope_position(
        self,
        aa_coords: AltazimutalCoords,
        started_at: datetime,
        decimal_places: int,
        speed: TelescopeSpeed = TelescopeSpeed.SPEED_TRACKING,
    ) -> EquatorialCoords:
        started_at = datetime.utcnow() if started_at is None else started_at
        eq_coords = self._altaz2radec(aa_coords=aa_coords, obstime=started_at)
        if speed is TelescopeSpeed.SPEED_NOT_TRACKING:  # type: ignore
            ...
            synced_eq_coords = EquatorialCoords(
                ra=round(ra, decimal_places), dec=round(eq_coords.dec, decimal_places)
            )
            logger.debug(
                f"equatorial coordinate for synced position when telescope is not tracking {synced_eq_coords}"
            )
            return synced_eq_coords
        logger.debug(
            f"equatorial coordinate for synced position when telescope is tracking  {eq_coords}"
        )
        synced_eq_coords = EquatorialCoords(
            ra=round(eq_coords.ra, decimal_places),
            dec=round(eq_coords.dec, decimal_places),
        )
        return synced_eq_coords

    def _radec2altaz(
        self, eq_coords: EquatorialCoords, obstime: datetime, decimal_places: int = 0
    ):
        ...
        coord = SkyCoord(
            ra=str(eq_coords.ra) + "h",
            dec=str(eq_coords.dec) + "d",
            equinox=equinox,
            frame="fk5",
        )
    ...
    def _altaz2radec(
        self, aa_coords: AltazimutalCoords, obstime: datetime, decimal_places: int = 0
    ):
        timestring = obstime.strftime(format="%Y-%m-%d %H:%M:%S")
        ...
        alt_az = SkyCoord(
            alt=aa_coords.alt * u.deg,
            az=aa_coords.az * u.deg,
            frame=aa,
            equinox=equinox,
        )  # type: ignore

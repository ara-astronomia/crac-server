...
class Telescope(TelescopeBase):
    ...
    def __init__(
        self,
        hostname=config.Config.getValue("hostname", "telescope"),
        port=config.Config.getInt("port", "telescope"),
    ) -> None:
    ...
    def retrieve(self) -> tuple:
        root = self.__call({"getProperties": {"version": 512, "device": self._name}})
    ...
    def _retrieve_status(
        self, aa_coords: AltazimutalCoords, root: Any
    ) -> TelescopeStatus:
    ...
    def __move(self, aa_coords: AltazimutalCoords, speed=TelescopeSpeed.SPEED_TRACKING):
        ...
        eq_coords = (
            self._altaz2radec(aa_coords, decimal_places=2, obstime=datetime.utcnow())
            if isinstance(aa_coords, (AltazimutalCoords))
            else aa_coords
        )

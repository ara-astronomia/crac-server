...
class Ups(UpsBase):
    ...
    def _read(self, device: str) -> dict[str,str]:
        ...
        return {
            # attributi statici: simulano un UPS NUT reale, che espone molti
            # più campi di quelli letti oggi da [ups_metrics] - servono a
            # esercitare davvero il filtro fatto da Ups.status_for(), non a
            # essere consumati direttamente
            ...
            'output.voltage': ups_config.get(device, "output.voltage", fallback='220'),
            ...
            # attributi dinamici, configurabili via ups.ini
            'input.voltage': voltage,
            'battery.charge': battery,
            'ups.status': ups_status,
            'output.current': current,
